...
//This handles the display of a Big M if present in the tableau
...
pub fn check_feasibility(tableau: &DMatrix<f64>) -> Result<(), SimplexError> {
    ...
    for i in 0..(rows - 1) {
        if tableau[(i, cols - 1)] < -1e-10 {
            // Provide more specific error message including the constraint number and value
...
pub fn augment_tableau(
    ...
    //Count the number of needed artificial, surplus, and slack variables correctly
